...
describe.sequential("publish packaging", () => {
  it("rewrites workspace package versions for canary publishing", async () => {
    ...
    try {
      ...
      expect(cliPackageJson.dependencies["@kienmatu/inkos-core"]).toBe("0.4.8-canary.7");
    } finally {
  ...
  it("links internal source dependencies through the workspace", async () => {
    ...
    expect(cliPackageJson.dependencies["@kienmatu/inkos-studio"]).toBe("workspace:*");
    expect(studioPackageJson.dependencies["@kienmatu/inkos-core"]).toBe("workspace:*");
  });
  ...
  it("accepts canonical workspace protocol in source manifests", async () => {
    ...
    try {
      ...
      await writeFile(
        join(tempCoreDir, "package.json"),
        `${JSON.stringify({ name: "@kienmatu/inkos-core", version: "0.5.1" }, null, 2)}\n`,
      );
      await writeFile(
        join(tempCliDir, "package.json"),
        `${JSON.stringify(
        ...
        `${JSON.stringify(
          {
            ...
            version: "0.5.1",
            dependencies: {
              "@kienmatu/inkos-core": "workspace:0.5.0",
            },
          },
          null,
          2,
  ...
  it("replaces workspace dependencies before npm pack", { timeout: CLI_PACK_TEST_TIMEOUT_MS }, async () => {
    ...
    try {
      ...
      expect(packedPackageJson.dependencies["@kienmatu/inkos-core"]).toBe(corePackageJson.version);
      expect(packedPackageJson.dependencies["@kienmatu/inkos-studio"]).toBe(studioPackageJson.version);
    } finally {
