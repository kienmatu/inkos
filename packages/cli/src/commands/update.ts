...
export const updateCommand = new Command("update")
  ...
  .action(async () => {
    try {
      ...
      const remoteVersion = execSync("npm view @kienmatu/inkos version", {
        encoding: "utf-8",
      }).trim();
      ...
      execSync("npm install -g @kienmatu/inkos@latest", { stdio: "inherit" });
      log(`Updated to ${remoteVersion}.`);
    } catch (e) {
      logError(`Update failed: ${e}`);
      log("You can also update manually: npm install -g @kienmatu/inkos@latest");
      process.exit(1);
    }
  });
