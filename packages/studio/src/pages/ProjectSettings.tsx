...
export function ProjectSettings({ nav, theme, t }: { nav: Nav; theme: Theme; t: TFunction }) {
  ...
  const importSkillFolder = async (files: FileList | null) => {
    ...
    await runSave("skill-import", async () => {
      ...
      await refetchSkills();
    }, tr("Skill 文件夹已导入", "Skill folder imported", "Đã nhập thư mục skill"));
  };
  ...
  return (
    <div className="space-y-8">
      ...
      <SettingsCard
        title={tr("Agent Skills", "Agent Skills", "Agent Skills")}
        description={tr(
          "导入标准 SKILL.md 专业能力包。Chat 可以按意图自主使用，也可以在输入框用 + 号强制启用。",
          "Import standard SKILL.md expertise packages. Chat can choose a skill from intent, or you can force one from the + menu.",
          "Nhập các gói skill chuẩn SKILL.md. Chat có thể tự chọn skill theo ý định, hoặc bạn có thể bắt buộc dùng một skill từ menu +.",
        )}
        icon={<Bot size={18} />}
      >
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-border/60 bg-secondary/20 p-3">
            <div>
              <div className="text-sm font-semibold">{tr("导入外部 Skill", "Import external skill", "Nhập skill bên ngoài")}</div>
              <p className="mt-1 text-xs text-muted-foreground">
                {tr(
                  ...
                  "Tương thích AgentSkills / OpenClaw: chọn thư mục đầy đủ có chứa SKILL.md. Các tệp tham khảo tĩnh sẽ được nhập theo; script sẽ không tự động chạy.",
                )}
              </p>
            </div>
            <button
            ...
            >
              <FolderUp size={16} />
              {saving === "skill-import"
                ? tr("导入中...", "Importing...", "Đang nhập...")
                : tr("选择 Skill 文件夹", "Choose skill folder", "Chọn thư mục skill")}
            </button>
            <input
          ...
          {skills.length === 0 ? (
            <p className="text-xs text-muted-foreground italic">{tr("还没有 Skill。", "No skills yet.", "Chưa có skill nào.")}</p>
          ) : (
                ...
                <div key={skill.id} className="rounded-xl border border-border/60 bg-secondary/20 p-3">
                  <div className="flex items-start gap-2">
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-1 text-xs leading-5 text-muted-foreground">{skill.description || tr("无说明", "No description", "Không có mô tả")}</p>
                    </div>
                    {skill.editable ? (
                      <button
                        type="button"
                        onClick={() => runSave(`delete-skill:${skill.id}`, async () => {
                          ...
                          await refetchSkills();
                        }, tr("Skill 已删除", "Skill deleted", "Đã xóa skill"))}
                        ...
                        aria-label={tr(`删除 ${skill.name}`, `Delete ${skill.name}`, `Xóa ${skill.name}`)}
                      >
                        <Trash2 size={14} />
                      </button>
                    ) : null}
      ...
      <SettingsCard
        title={tr("提示词", "Prompt packs", "Gói prompt")}
        description={tr(
          "集中查看和调整内置提示词。修改会保存为项目级覆盖文件，不会改动内置默认值。",
          "Review and tune built-in prompt packs. Edits are saved as project overrides without changing the defaults.",
          "Xem và tinh chỉnh các gói prompt có sẵn tại một nơi. Thay đổi được lưu thành tệp ghi đè cấp dự án, không ảnh hưởng đến giá trị mặc định.",
        )}
          ...
          <div className="rounded-xl border border-border/60 bg-secondary/20 p-3">
            {promptGroups.length === 0 ? (
              <p className="text-xs text-muted-foreground italic">
                {tr("没有可编辑提示词。", "No prompt packs available.", "Không có gói prompt nào để chỉnh sửa.")}
              </p>
            ) : (
          ...
          <div className="rounded-xl border border-border/60 bg-secondary/20 p-3">
            {selectedPrompt ? (
              <div className="space-y-3">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    ...
                    <div className="mt-1 text-xs text-muted-foreground">
                      {tr("当前来源", "Source", "Nguồn")}: {selectedPrompt.source}
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      }, tr("提示词已恢复默认", "Prompt reset to default", "Đã khôi phục prompt về mặc định"))}
                    ...
                    >
                      <RotateCcw size={14} />
                      {tr("恢复默认", "Reset", "Khôi phục mặc định")}
                    </button>
                    <button
                      type="button"
                      onClick={() => runSave(`prompt:${selectedPrompt.id}`, async () => {
                        ...
                        await refetchPromptPacks();
                      }, tr("提示词已保存", "Prompt saved", "Đã lưu prompt"))}
                ...
                <details className="rounded-xl border border-border/50 bg-background/50 p-3">
                  <summary className="cursor-pointer text-sm font-semibold text-muted-foreground">
                    {tr("查看内置默认", "View built-in default", "Xem bản mặc định có sẵn")}
                  </summary>
              ...
              <p className="text-sm text-muted-foreground">
                {tr("选择左侧提示词后编辑。", "Select a prompt on the left to edit it.", "Chọn một prompt ở bên trái để chỉnh sửa.")}
              </p>
            )}
          </div>
        </div>
      ...
      <SettingsCard
        title={tr("联网研究搜索服务", "Research Search Provider", "Dịch vụ tìm kiếm nghiên cứu")}
        description={tr(
          "给 research_web 配置外部搜索 API。未配置时仍可用服务器环境变量 TAVILY_API_KEY 作为兜底。",
          "Configure the external search API used by research_web. If unset, the server may still use TAVILY_API_KEY as a fallback.",
          "Cấu hình API tìm kiếm bên ngoài cho research_web. Nếu chưa cấu hình, server vẫn có thể dùng biến môi trường TAVILY_API_KEY làm phương án dự phòng.",
        )}
        icon={<Search size={18} />}
      >
        <label className="flex items-center gap-2 text-sm">
          ...
          />
          {tr("启用项目级搜索配置", "Enable project-level search config", "Bật cấu hình tìm kiếm cấp dự án")}
        </label>
        <Collapse open={researchSearch.enabled}>
          <div className="grid gap-2 pt-1 md:grid-cols-2">
            <label className="space-y-1 text-xs text-muted-foreground">
              <span>{tr("搜索服务", "Provider", "Nhà cung cấp")}</span>
              <select
            ...
            <label className="space-y-1 text-xs text-muted-foreground">
              <span>{tr("API Key 环境变量名", "API key env var", "Tên biến môi trường API key")}</span>
              <input
            ...
            <label className="space-y-1 text-xs text-muted-foreground md:col-span-2">
              <span>{tr("Base URL（可选，自定义兼容端点）", "Base URL (optional custom compatible endpoint)", "Base URL (tùy chọn, endpoint tương thích tùy chỉnh)")}</span>
              <input
            ...
            <label className="space-y-1 text-xs text-muted-foreground md:col-span-2">
              <span>{tr("API Key（可选；留空则读环境变量）", "API key (optional; leave blank to use env var)", "API key (tùy chọn; để trống sẽ dùng biến môi trường)")}</span>
              <input
                value={researchSearch.apiKey}
                onChange={(e) => setResearchSearch((prev) => ({ ...prev, apiKey: e.target.value }))}
                type="password"
                placeholder={tr("可直接填 key，或只填环境变量名", "Paste key, or use env var only", "Dán key trực tiếp, hoặc chỉ điền tên biến môi trường")}
                className={`${fieldClass} font-mono`}
              />
            </label>
          </div>
