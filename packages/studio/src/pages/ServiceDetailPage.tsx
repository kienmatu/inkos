...
export function ServiceDetailPage({ serviceId, nav }: { serviceId: string; nav: Nav }) {
  ...
  const label = isCustom ? (customName || persistedCustomName || tr("自定义服务", "Custom service", "Dịch vụ tùy chỉnh")) : (svc?.label ?? serviceId);
  ...
  const handleTest = async () => {
    ...
    if (!trimmedKey && !isCustom && !apiKeyOptional) {
      setStatus({ state: "error", message: tr("请先输入 API Key", "Enter an API key first", "Vui lòng nhập API Key trước") });
      return;
    }
    if (isCustom && !baseUrl.trim()) {
      setStatus({ state: "error", message: tr("请先填写 Base URL", "Enter a base URL first", "Vui lòng nhập Base URL trước") });
      return;
    }
    ...
    try {
      ...
      if (result.ok) {
        ...
        setStatus({ state: "error", message: result.error ?? tr("连接失败", "Connection failed", "Kết nối thất bại") });
        clearStoreModels(effectiveServiceId);
      }
    } catch (e) {
      setVerifiedProbe(null);
      setStatus({ state: "error", message: e instanceof Error ? e.message : tr("连接失败", "Connection failed", "Kết nối thất bại") });
    }
  };

  const handleDelete = async () => {
    if (!window.confirm(tr(`删除“${label}”的配置和密钥？`, `Delete the config and key for “${label}”?`, `Xóa cấu hình và key của "${label}"?`))) return;
    ...
    try {
      ...
      setStatus({ state: "error", message: e instanceof Error ? e.message : tr("删除失败", "Delete failed", "Xóa thất bại") });
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setStatus({ state: "error", message: e instanceof Error ? e.message : tr("保存失败", "Save failed", "Lưu thất bại") });
    }
  };
  ...
  return (
    <div className="max-w-xl mx-auto space-y-6">
      ...
      >
        <ArrowLeft size={14} />
        {tr("返回服务商管理", "Back to providers", "Về trang quản lý nhà cung cấp")}
      </button>
      ...
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-2 gap-4">
            <Field label={tr("服务名称", "Service name", "Tên dịch vụ")}>
              <input type="text" value={customName} onChange={(e) => setCustomName(e.target.value)}
                placeholder={tr("例如：本地 Ollama", "e.g. local Ollama", "Ví dụ: Ollama cục bộ")} className="w-full rounded-lg border border-border/60 bg-background px-3 py-2 text-sm" />
            </Field>
        ...
        {/* API Key */}
        <Field label={apiKeyOptional ? tr("API Key（可选）", "API key (optional)", "API Key (tùy chọn)") : "API Key"}>
          <div className="relative">
            <input
              type={showKey ? "text" : "password"} value={apiKey}
              onChange={(e) => setApiKey(e.target.value)} placeholder={apiKeyOptional ? tr("本地服务可留空", "Optional for local service", "Có thể để trống cho dịch vụ cục bộ") : "sk-..."}
        ...
        <div className="flex items-center gap-2">
          <button onClick={handleTest} disabled={isBusy}
            ...
            {tr("测试连接", "Test connection", "Kiểm tra kết nối")}
          </button>
          ...
          {(isConnected || isCustom) && (
            <button onClick={handleDelete} disabled={isBusy}
              ...
              <Trash2 size={12} />
              {tr("删除配置", "Delete config", "Xóa cấu hình")}
            </button>
          )}
          {/* Status feedback */}
          {status.state === "connected" && (
            <span className="text-xs text-emerald-500">
              {tr(`连接成功，${models.length} 个模型`, `Connected, ${models.length} models`, `Đã kết nối, ${models.length} model`)}
              {detectedModel
                ? tr(
                    ...
                    `, đã tự khớp ${detectedModel}${detectedConfig ? ` / ${detectedConfig.apiFormat === "responses" ? "Responses" : "Chat"} / ${detectedConfig.stream ? "streaming" : "non-streaming"}` : ""}`,
                  )
                : ""}
            </span>
          )}
          ...
          {status.state === "saved" && (
            <span className="text-xs text-emerald-500">{tr("已保存", "Saved", "Đã lưu")}</span>
          )}
        </div>

        <div className="grid grid-cols-2 gap-4">
          <Field label={tr("协议类型", "Protocol", "Loại giao thức")}>
            <select
              value={apiFormat}
          ...
          <Field label={tr("流式响应", "Streaming", "Phản hồi streaming")}>
            <label className="flex h-10 items-center gap-2 rounded-lg border border-border/60 bg-background px-3 text-sm">
              ...
              />
              <span>{stream ? tr("开启", "On", "Bật") : tr("关闭", "Off", "Tắt")}</span>
            </label>
          </Field>
        </div>
        ...
        <div className="space-y-2">
          <p className="text-xs text-muted-foreground/70 font-medium uppercase tracking-wider">
            {tr(`模型目录（${models.length}）`, `Model catalog (${models.length})`, `Danh mục model (${models.length})`)}
          </p>
          <div className="flex gap-2">
            <input
              ...
              placeholder={tr("输入模型 ID，例如 gemini-3.1-pro", "Enter a model ID, e.g. gemini-3.1-pro", "Nhập model ID, ví dụ gemini-3.1-pro")}
          ...
          <p className="text-xs text-muted-foreground/60">
            {tr("测试连接发现的模型和手动添加的模型都会在保存后持久化；内置目录只作为兜底。", "Discovered and manually added models are persisted on save; the built-in catalog is only a fallback.", "Model tìm thấy khi kiểm tra kết nối và model thêm thủ công đều được lưu lại; danh mục có sẵn chỉ dùng khi không có gì khác.")}
          </p>
          {hasModelCatalog && (
          <div className="space-y-2">
            {models.length > 0 ? (
                ...
                {models.map((m) => (
                  <span key={m.id} className="inline-flex items-center gap-1 text-[11px] px-2.5 py-1 rounded-md bg-emerald-500/[0.06] text-emerald-600 dark:text-emerald-400 border border-emerald-500/15">
                    ...
                    <button
                      ...
                      aria-label={tr(`移除模型 ${m.id}`, `Remove model ${m.id}`, `Xóa model ${m.id}`)}
              ...
              <p className="text-xs text-muted-foreground/60">{tr("点击“测试连接”查看可用模型", "Click “Test connection” to list available models", "Nhấn \u201cKiểm tra kết nối\u201d để xem model khả dụng")}</p>
            )}
          </div>
          )}
        </div>
        ...
        <details className="group pt-2 border-t border-border/20">
          <summary className="text-xs text-muted-foreground/60 cursor-pointer select-none hover:text-muted-foreground transition-colors py-2">
            {tr("高级参数", "Advanced", "Tham số nâng cao")}
          </summary>
