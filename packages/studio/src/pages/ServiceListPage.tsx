...
function CoverConfigCard() {
  ...
  useEffect(() => {
    ...
    void fetchJson<CoverConfigPayload>("/cover/config")
      ...
      .catch((error) => {
        if (cancelled) return;
        setStatus("error");
        setMessage(error instanceof Error ? error.message : tr("读取封面配置失败", "Failed to load cover config", "Không đọc được cấu hình ảnh bìa"));
      });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setStatus("saved");
      setMessage(tr("封面配置已保存", "Cover config saved", "Đã lưu cấu hình ảnh bìa"));
    } catch (error) {
      setStatus("error");
      setMessage(error instanceof Error ? error.message : tr("保存封面配置失败", "Failed to save cover config", "Không lưu được cấu hình ảnh bìa"));
    }
  };
  ...
  return (
    <section className="rounded-xl border border-border/50 bg-card/50 p-4 space-y-3">
      <div className="flex items-start justify-between gap-3">
        <div>
          <h2 className="text-sm font-medium text-foreground">{tr("封面生成", "Cover generation", "Tạo ảnh bìa")}</h2>
          <p className="mt-1 text-xs text-muted-foreground/70">
            {tr(
              ...
              "Chỉ cấu hình nhà cung cấp và model cho ảnh bìa; kích thước ảnh bìa do prompt ảnh bìa truyện ngắn và mặc định nội bộ xử lý.",
            )}
          </p>
        </div>
        {selected?.connected && (
          <span className="rounded-full bg-emerald-500/10 px-2 py-0.5 text-[10px] font-medium text-emerald-500">
            {tr("已有密钥", "Key saved", "Đã có key")}
          </span>
        )}
      </div>
      ...
      <div className="grid gap-3 md:grid-cols-2">
        ...
        <label className="space-y-1.5">
          <span className="block text-xs font-medium text-muted-foreground/70">{tr("封面模型", "Cover model", "Model ảnh bìa")}</span>
          <select
            value={model}
      ...
      <label className="space-y-1.5">
        ...
        <span className="block text-[11px] leading-5 text-muted-foreground/55">
          {tr(
            ...
            "Để trống sẽ dùng địa chỉ mặc định của dịch vụ; giá trị tùy chỉnh sẽ là root API tạo ảnh bìa.",
          )}
        </span>
      </label>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        >
          ...
          {tr("保存封面配置", "Save cover config", "Lưu cấu hình ảnh bìa")}
        </button>
        {message && (
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      ...
      <label className="inline-flex cursor-pointer select-none items-center gap-2 text-xs text-muted-foreground">
        ...
        />
        <span>{tr("只看已连接", "Connected only", "Chỉ hiện đã kết nối")} ({connectedCount})</span>
      </label>
      ...
      {!loading && filtered.length === 0 && filteredCustom.length === 0 && !canCreateCustom && (
        <div className="rounded-lg border border-dashed border-border/40 p-8 text-center text-sm text-muted-foreground">
          {tr("没有匹配的服务商", "No matching providers", "Không có nhà cung cấp phù hợp")}
        </div>
      )}
    </div>
  );
