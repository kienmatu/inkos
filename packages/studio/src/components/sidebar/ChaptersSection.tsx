...
export function ChaptersSection({ bookId }: ChaptersSectionProps) {
  ...
  return (
    <SidebarCard title={tr("章节", "Chapters", "Chương")}>
      {chapters.length === 0 ? (
        <p className="text-[15px] leading-6 text-muted-foreground/50 italic">
          {tr("暂无章节", "No chapters", "Chưa có chương nào")}
        </p>
      ) : (
            ...
            return (
              <li
                ...
                <span className="truncate flex-1">
                  {String(ch.number).padStart(2, "0")} {ch.title || tr(`第${ch.number}章`, `Chapter ${ch.number}`, `Chương ${ch.number}`)}
                </span>
