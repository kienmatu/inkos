...
export function ProjectArtifactDrawer() {
  ...
  return (
    <div className="fixed inset-0 z-[80] flex justify-end bg-background/35 backdrop-blur-[2px]">
      <button
        type="button"
        aria-label={tr("关闭生成物预览", "Close artifact preview", "Đóng bản xem trước bản dựng")}
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-6 py-5">
          {loading ? (
            <div className="flex h-full items-center justify-center text-muted-foreground">
              ...
              {tr("正在读取生成物...", "Loading artifact...", "Đang tải bản dựng...")}
            </div>
          ) : editing ? (
            <textarea
              value={draft}
            ...
            <div className="rounded-xl border border-dashed border-border/55 px-4 py-8 text-center text-[14px] text-muted-foreground">
              {tr("没有可预览内容。", "Nothing to preview.", "Không có nội dung để xem trước.")}
            </div>
          )}
        </div>
      </aside>
